...
const TaskListTable = ({ tableData }) => {
    const getStatusBadgeColor = (status) => {
        switch (status) {
            ...
            case 'Pending': return 'bg-purple-100 tet-purple-500 border border-purple-200';
    ...
    const getpriorityBadgeColor = (priority) => {
        switch (priority) {
            ...
            default: return 'bg-green-100 tet-gray-500 border border-gray-200'
        }
    }
    return (
            ...
            <table className='min-w-full'>
                <thead>
                    <tr className='text-left'>
                        ...
                        <th className='py-3 px-4 text-gray-800 font-medium text-[13px] hidden md:table-caption'>Created On</th>
                    </tr>
                </thead>
                <tbody>
                    {
                        tableData.map((task) => {
                            <tr key={task._id} className='border-t border-gray-200'>
                                ...
                                <td className={`px-2 py-1 text-xs rounded inline-block ${getpriorityBadgeColor(task.priority)}`}>{task.priority}</td>
                                <td className='px-2 py-4 text-gray-700 text-[13px] text-nowrap hidden md:table-cell'>
